setwd("D:/School/Research Methodology/Assignments/Assignment") # Adapt to your computer

rm(list = ls())   #Clear R's memory

library(ggcorrplot)
library(psych)
library(hydroGOF) # Library no longer available on newer versions of R
...
#New findings
cor <- cor(DATASET)
ggcorrplot(cor) #Save as a PNG file
...
m10a <- lm(sqrt(TOTAL_VOLUME)~sqrt(BA*H), data=datanew)
...
#Residual figures
plot(m6$fitted.values,m6$residuals, abline(h=0))
plot(m7$fitted.values,m7$residuals,ylab='Residuals', xlab='Fitted values',main='1st Model Residuals vs Fitted', abline(h=0))
plot(m8$fitted.values,m8$residuals, abline(h=0))
plot(m9$fitted.values,m9$residuals, abline(h=0))
plot(m10$fitted.values,m10$residuals, abline(h=0))
plot(m10a$fitted.values,m10a$residuals, abline(h=0))
...
#Residual figures
plot(m11$fitted.values,m11$residuals, abline(h=0))
plot(m12$fitted.values,m12$residuals,ylab='Residuals', xlab='Fitted values',main='1st Model Residuals vs Fitted', abline(h=0))
plot(m13$fitted.values,m13$residuals, abline(h=0))
plot(m14$fitted.values,m14$residuals, abline(h=0))
plot(m15$fitted.values,m15$residuals, abline(h=0))
...
#FINAL 3: m11, m14, and m16 (Reasons mentioned above are repeated below for easy comparison)
hist(m11$residuals)
hist(m14$residuals)
hist(m16$residuals)
summary(m16) 
...
#We use our new model (m16) to predict volume and save as a new column
mod.pred <- predict((1.079893 + 0.551283*sqrt(BA*D))^2,DATASET)
mod.pred<-predict(model.final,DATASET)
...
head(obs.pred) # Good predictions, close to originally observed values
...
pbias(obs.pred1$Modeled, obs.pred1$Original) # Not working becasue of missing library
